...
def afk(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(tld(chat.id, f"{fname} Bentar Mau Ngewe!"))
...
def no_longer_afk(bot: Bot, update: Update):
    ...
    if res:
        ...
        try:
            update.effective_message.reply_text(tld(chat.id, f"{firstname} Udah balik nih kangen ga?"))
        except:
            return
...
def check_afk(bot, update, user_id, fst_name):
    ...
    if sql.is_afk(user_id):
        ...
        if not user.reason:
            ...
            res = tld(chat.id, f"{fst_name} Sedang AFK! Alasan :\n{user.reason}")
